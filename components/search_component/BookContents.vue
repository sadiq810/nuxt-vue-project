    ...
    <div class="sec-title spaceLR-main mb-4">
      <div class="secTitleL">
        <h1 class="h3">
          ...
          <span class="strong6">{{ $t('book_search_result') }}</span>
    ...
    <ul class="fatawa-list">
      <li class="mb-3" v-for="item in getSearchResult" :key="item._id">
        <div class="box box-inner-big">
          <nuxt-link class="thumbnail-item thumbnail-item-full" :to="localePath({name: 'books-id', params:{id: item.book_id}, query: {index: item.id, term: getSearchStr}})">
...
export default {
  ...
  methods: {
    loadData(page) {
      this.$store.dispatch('performSearch', {keyword: this.getSearchStr, pageNo: page});
    }
  }
}
</script>

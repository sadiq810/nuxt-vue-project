<template>
  <div id="MainContent" :class="'book-'+bgColor +' book-content book-content-view'">
    ...
    <BreadCrumbComponent :title="data ? data.title : ''" :innerPage="$t('articles_list')" :heading="$t('articles_list')" innerSlug="posts"/>
            ...
            <article class="bookPage box mb-4">
              ...
              <div class="bookViewContent" id="bookViewContent">
                <div class="box-post" id="printMe">
                  <div style="margin-bottom: 30px; text-align: right;" v-html="data.details" id="inner--contents"></div>
                </div>
              </div>
            </article>
          </div>
                  ...
                  <ul>
                    <li v-for="item in related" :key="item.id">
                      <div class="ml-item"> <i class="ico-file ml-item-ico"></i>
                        <nLink :to="localePath(`/posts/${item.id}`)">{{ item.title }}</nLink>
                      </div>
                    </li>
                  </ul>
                </div>
...
export default {
  ...
  async asyncData({app, params, store, query}) {
    ...
    if (store.state.posts && store.state.posts.data && store.state.posts.data.length > 0)
      result = store.state.posts.data.find(f => f.id == params.id);

    if (result)
      return {data: result, related: store.state.posts.data.filter(f => f.id != result.id)};
    else {
      try {
        let {data} = await app.$axios.get(`/single-post/${params.id}`+(query.term ? '?term='+encodeURI(query.term): ''));
        return {data: data.data, related: data.related};
      } catch (error) {
        console.log('Could not fetch single post, in detail post.', error);
      }//..... end of try-catch() .....//
    }//..... end if-else() .....//
  },
  ...
  mounted() {
    this.$store.dispatch('recordPageView', {type: 'post', id: this.$route.params.id});
  },
  ...
  computed: {
    ...
    getTitleForPage() {
      return this.$t('menu.posts') + " - " +this.data.title;
    },
  ...
  head() {
    ...
    let url = "https://ibn-mahmoud.com/posts/"+this.$route.params.id;

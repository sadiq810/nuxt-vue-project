<template>
  <div id="MainContent" :class="'book-'+bgColor +' book-content book-content-view'">
    <div class="MainContent-Container">
      ...
      <BreadCrumbComponent :title="data ? data.title : ''" :innerPage="$t('menu.sheikh_biography')" innerSlug="biography" :heading="$t('menu.sheikh_biography')"/>
        ...
        <article class="bookPage mb-4">
          ...
          <div class="bookViewContent" id="bookViewContent">
            <div class="box-post box-post-sm" id="printMe">
              <div style="margin-bottom: 30px; text-align: right;" v-html="data ? data.detail: ''"></div>
            </div>
...
export default {
  ...
  async asyncData({app, params, store}) {
    ...
    if (result)
      ...
      try {
        let {data} = await app.$axios.get(`/biography-by-id/${params.id}`);
        return {data: data};
      } catch (error) {
        console.log('Could not fetch single biography, in detail page.', error);
      }//..... end of try-catch() .....//
    }//..... end if-else() .....//
  },
  ...
  mounted() {
    this.$store.dispatch('recordPageView', {type: 'biography', id: this.$route.params.id});
  },
  ...
  computed: {
    ...
    getTitleForPage() {
      return this.$t('menu.sheikh_biography') + " - " +this.data.title;
    },

    getDescriptionForSeo() {
      let des = this.data.detail;
      if (des) {
        des = stripTags(des);
  ...
  head() {
    ...
    let url = "https://ibn-mahmoud.com/biography/"+this.$route.params.id;

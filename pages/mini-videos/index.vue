<template>
  <div id="MainContent" class=" player-atBottom">
    <div class="MainContent-Container">
      <BreadCrumbComponent :title="$t('videos_library')" :heading="$t('menu.mini_videos')"/>
  ...
  export default {
    ...
    async fetch({store}) {
      //if (store.state.videos.length == 0)
        await store.dispatch('fetchVideos', {page: 1, type: 'short'})
    },
    ...
    methods: {
      async loadVideos(page = 1) {
        let sort = this.sort;
        await this.$store.dispatch('fetchVideos', {page, sort, type: 'short'});
      },
    ...
    head() {
      ...
      let url = "https://ibn-mahmoud.com/mini-videos";

      return {
        title: this.$t('menu.mini_videos')+ " - " +title,
        htmlAttrs: {
          ...i18nSeo.htmlAttrs,

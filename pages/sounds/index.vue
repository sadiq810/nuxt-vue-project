<template>
  <div id="MainContent" class=" player-atBottom">
    ...
    <div class="MainContent-Container">
      <BreadCrumbComponent :title="$t('audio_library')" :heading="$t('audio_library')"/>
      ...
      <div class="sec-container InnerPage-Box" style="margin-top: -60px;">
        <div class="CategoryWithFilter">
            ...
            <div class="box book-categories">
              <div class="head-title p-3">
                <div class="d-flex align-items-center mb-4">
                  <h4 class="main-color strong6 mb-0">{{ $t('audio_library') }}</h4>
              ...
              <div class="sideBookmarks-scrollBox">
                <div class="underline-list multiLevels-menu">
                  <ul ref="menuList">
                    <li v-for="f in filterList" :key="f.id" :class="[f.id==getSelectedCategory ? 'current' : '']">
                      <div class="ml-item">
                        <i class="ico-volume ml-item-ico"></i>
                        <a href="javascript:;" @click="setSoundCategory(f.id)">{{ f.name }}</a></div>
                    </li>
                  </ul>
                </div>
              </div>
          ...
          <div class="Category-MainContent">
            <div class="box">
              <div class="box-inner border-bottom">
                <div class="sec-title spaceLR-main mb-0">
                  <div class="secTitleL">
                    <h1 class="h3"><span class="strong6">{{ categoryTitle ? categoryTitle.name : 'كل الأصوات' }}</span>
                    </h1>
                  </div>
              ...
              <div class="sound-list mb-4">
                ...
                <div class="sound-list-content">
                  <ul v-if="getSounds.data.length > 0" class="playlist-list list list-items list-items-sm" :style="{'max-height': height+'px', 'overflow-y':'auto'}">
                    <Audios v-for="(audio,index) in getSounds.data" :key="index" :audio="audio" @playThis="playThis(index, audio.id)" @playNext="playNext" :showList="showList" :activeindex="activeindex"/>
                  </ul>
            ...
            <PaginationComponent :searchMeta="getSounds" @onPageChange="loadAudios"/>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  async fetch({store}) {
    await store.dispatch('fetchSounds');
  },

  computed: {
    ...mapGetters(['getSounds', 'getSoundCategory', 'getSelectedCategory', 'getSiteDirection', 'getBreadcrumbs']),
    categoryTitle() {
      return this.getSoundCategory ? this.getSoundCategory.find(f => f.id == this.getSelectedCategory) : undefined;
    },
  ...
  updated() {
    if (mainPlayerInstance) {
      let playlist = [];
      this.getSounds.data.map(function (value, key) {
        playlist.push({
  ...
  methods: {
    setSoundCategory(id) {
      this.$store.commit('SET_SELECTED_SOUND_CATEGORY', id);
      this.loadAudios(1);
    },
    ...
    loadAudios(page = 1, id = null) {
      let category_id = this.getSelectedCategory ? this.getSelectedCategory : 0;
      let sort = this.sort;
      this.$store.dispatch('fetchSounds', {page, sort, category_id});
    },
    sortChange(value) {
      this.sort = value;
      this.loadAudios(1)
  ...
  head() {
    ...
    let url = "https://ibn-mahmoud.com/sounds";

    return {
      title: this.$t('menu.sounds')+ " - " +title,
      htmlAttrs: {
        ...i18nSeo.htmlAttrs,
